...
const Register = () => {
  ...
  const submit=async (e)=>{
    ...
    register({title,fullName,email,password:pass})
    .then((res)=>{
      toast.success("Registration successful");
      setEmail("");setPass("");setFullName("");setTitle("");
    })
    .catch((err)=>{
    ...
    <div className='w-full h-auto flex justify-center'>
      <div className=' w-full m-4 md:m-0 md:w-1/2 h-auto md:my-16 shadow-lg bg-indigo-50'>
        ...
        <form onSubmit={(e)=>{submit(e)}}  className='m-1 md:m-2 flex flex-col justify-center'>
          ...
          <input value={fullName} onChange={(e)=>{setFullName(e.target.value)}} className='outline-0 border-slate-600 border-b-[1px] p-1 md:p-2 my-1 md:my-2 mx-2 md:mx-4 text-lg md:text-xl' type="text" placeholder='Enter your full name'  autoComplete="off" />
